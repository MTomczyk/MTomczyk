...
import sort.functions.TApproximateUpdateFunction;
...
public class LaumannsApproximationGenetic implements IGenetic
{
    ...
    public LaumannsApproximationGenetic(Params params)
    {
    ...
    public void reproduce(Integer generation)
    {
        ...
        // LAUMANNS
        this._archive = TApproximateUpdateFunction.getSet(this._archive, reproduction.get(0), _criterion, _t, _epsilon);
        this._specimen = this._archive;
        //System.out.println(this._specimen.size());
    ...
    public void kill(int generation)
    {
        long startTime = System.nanoTime();
        int number = 1;
        if (_specimen.size() <= _populationSize) number = 0;

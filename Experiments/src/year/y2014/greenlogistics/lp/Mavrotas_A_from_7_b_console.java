 ...
 */
public class Mavrotas_A_from_7_b_console
{
    public static void main(String args[])
    {
        ...
        double resultParetoSize[] = new double[steps];
        double resultsBestWSMDist[] = new double[steps];
        double resultsBestECMDist[] = new double[steps];
        double resultsBestUtil[] = new double[steps];
        double resultsAverageUtil[] = new double[steps];
        ...
        for (int step = 0; step < steps; step++)
        {
            ...
            if (pareto.size() < 2)
            {
                ...
                {
                    resultParetoSize[step2] = resultParetoSize[step - 1];
                    resultsBestECMDist[step2] = resultsBestECMDist[step - 1];
                    resultsBestWSMDist[step2] = resultsBestWSMDist[step - 1];
                    resultsBestUtil[step2] = resultsBestUtil[step - 1];
                    resultsAverageUtil[step2] = resultsAverageUtil[step - 1];
                }
                break;
            }

            //ArrayList<IAlternative> alternatives = choice.getAlternativesToCompare(pareto, new FromSpecimen(), dm);
            ArrayList<IAlternative> alternatives = new ArrayList<>(7);
            {
                ArrayList<Range> ranges = RangeMaker.getRange(pareto, criteria);
                ArrayList<INormalization> normalizations = new ArrayList<>();
                //System.out.println("------------ " + ranges.get(0).left + " " + ranges.get(0).right);

                for (int o = 0; o < 3; o++)
                {
                    if (ranges.get(o).getRange() < -Common.EPSILON * 2.0d)
                        System.out.println(ranges.get(o).getRange() + " " + ranges.get(o).left + " " + ranges.get(o).right);
                    if (Math.abs(ranges.get(o).getRange()) > Common.EPSILON * 2.0d)
                        normalizations.add(new MinMaxNormalization(ranges.get(o).left, ranges.get(o).right));
                    else
                    {
                        double mi = ranges.get(o).left - 3.0d * Common.EPSILON;
                        double ma = ranges.get(o).left + 3.0d * Common.EPSILON;
                        normalizations.add(new MinMaxNormalization(mi, ma));
                    }
                ...
                IDistance distance = new Euclidean(p);
                // MIDDLE
                double middlePointData[] = {ranges.get(0).left + ranges.get(0).getRange() / 2.0d,
                        ranges.get(1).left + ranges.get(1).getRange() / 2.0d,
                        ranges.get(2).left + ranges.get(2).getRange() / 2.0d};
                Point middlePoint = new Point(middlePointData);
                double min = Common.MAX_DOUBLE;
                ISpecimen tmp = null;
                for (ISpecimen s: pareto)
                {
                    double d[] = s.getAlternative().getEvaluationVector(criteria);
                    Point sPoint = new Point(d);
                    if (distance.getDistance(middlePoint, sPoint) < min)
                    {
                        min = distance.getDistance(middlePoint, sPoint);
                        tmp = s;
                    }
                }
                alternatives.add(tmp.getAlternative());
                tmp = null;
                // EXTREME
                for (int i = 0; i < 3; i++)
                {
                    min = Common.MAX_DOUBLE;
                    for (ISpecimen s: pareto)
                    {
                        double e = s.getAlternative().getEvaluationAt(criteria.get(i));
                        if (Math.abs(e - ranges.get(i).left) < min)
                        {
                            min = Math.abs(e - ranges.get(i).left);
                            tmp = s;
                        }
                    }
                    alternatives.add(tmp.getAlternative());
                ...
                {
                    // -----------------------------------
                    double ref[] = {0.0d, 0.0d, 0.0d};
                    for (int j = 0; j < 3; j++)
                        ref[j] += alternatives.get(0).getEvaluationAt(criteria.get(j));
                    if (i == 0)
                    {
                        for (int j = 0; j < 3; j++)
                        {
                            ref[j] += alternatives.get(1).getEvaluationAt(criteria.get(j));
                            ref[j] += alternatives.get(2).getEvaluationAt(criteria.get(j));
                        }
                    }
                    else if (i == 1)
                    {
                        ...
                        {
                            ref[j] += alternatives.get(1).getEvaluationAt(criteria.get(j));
                            ref[j] += alternatives.get(3).getEvaluationAt(criteria.get(j));
                        }
                    }
                    else
                    {
                    ...
                    for (ISpecimen s: pareto)
                    {
                        double d[] = s.getAlternative().getEvaluationVector(criteria);
                        Point sPoint = new Point(d);
                        if (distance.getDistance(new Point(ref), sPoint) < min)
                        {
                            min = distance.getDistance(new Point(ref), sPoint);
                            tmp = s;
                        }
                    }
                    alternatives.add(tmp.getAlternative());
            ...
            resultParetoSize[step] = pareto.size();
            resultsBestECMDist[step] = distECM;
            resultsBestWSMDist[step] = distWSM;
            resultsBestUtil[step] = bestUtil;
            resultsAverageUtil[step] = averageUtil;
        ...
        String name = String.format("%d_%.2f", TRIAL, ALPHA);
        String path = "Results/year/y2015/greenlogistics/11_2015/mavrotas_7b/";
        FileUtil.createDir(path);

        File file = FileUtil.getFile(path + "/" + name + ".txt");
        ...
        for (int s = 0; s < steps; s++)
        {
            String f = String.format("%d: %f %f %f %f %f %f %f %f %f %f", s,
                    resultParetoSize[s],
                    resultParetoSize[s],
                    resultsBestECMDist[s],
                    resultsBestECMDist[s],
                    resultsBestWSMDist[s],
                    resultsBestWSMDist[s],
                    resultsBestUtil[s],
                    resultsBestUtil[s],
                    resultsAverageUtil[s],
                    resultsAverageUtil[s]);
            System.out.println(f);
            FileUtil.writeToFile(bw, f);
        }

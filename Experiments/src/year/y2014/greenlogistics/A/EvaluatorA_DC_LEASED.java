...
public class EvaluatorA_DC_LEASED implements IEvaluator
{
			...
			double criterionDC[], double trDC[], double citiesEP[], double tStorage[][], DataA p)
	{
		...
		for (Integer v : connection)
		{
			...
			for (int i = 0; i < DC.size(); i++)
			{
				...
				storageCost[i] = this.calcStorageLeased(dmd, 0, cityDC, p);
	...
	public void evaluate(ArrayList<ICriterion> criteria, ISpecimen specimen, Object problem, boolean log)
	{
		...
		for (double aDc_p : dc_p)
		{
			...
			if (tStorage[1][cityB] < 1.0)
				continue;
			costWarehouse += this.calcStorageLeased(tStorage[1][cityB], 0, cityB, p);
		}
    ...
    private double calcStorageLeased(double demand, double storage, int city, DataA p)
    {
        double st = storage + demand;

        for (int i = 0; i < 5; i++)
        {
            if (st < p.storageCostLeasing[city][2 * i])
            {
                return p.storageCostLeasing[city][2 * i + 1];
            }
        }

package year.y2014.greenlogistics.A;
...
public class EvaluatorA implements IEvaluator
{
	...
	private double[][] updateFirst(int cityRM, int criterion, int tr, double demand, double citiesDC[],
			double criterionDC[], double trDC[], double citiesEP[], double tStorage[][], DataA p)
	{
	...
	public void evaluate(ArrayList<ICriterion> criteria, ISpecimen specimen, Object problem, boolean log)
	{
		...
		for (int i = 0; i < rm_p.length; i++)
		{
			...
			if (log)
			{
				System.out.printf("%s -> %s: (%d)(%f) %.5f %.5f %.5f\n", p.names[cityRM],
						p.names[(int) result[0][7]], (int) result[0][8], p.distances[cityRM][(int) result[0][7]], result[0][0], result[0][1],
						result[0][2]);
			}
		...
		if (log) System.out.printf(" DC -> EP\n");
		// OBLICZ DC - EP
		double war = 0;
		for (int i = 0; i < dc_p.length; i++)
		{
			...
			{
				System.out.printf("%s -> %s: (%d)(%f) %.5f %.5f %.5f\n", p.names[cityDC],
						p.names[(int) result[0][7]], (int) result[0][8],p.distances[cityDC][(int) result[0][7]], result[0][0], result[0][1],
						result[0][2]);
			}
		...
		else
		{
			...
			if (log)
			{
				...
				System.out.printf("%f %f %f\n", specimen.getAlternative().getEvaluationAt(criteria.get(0)),
						specimen.getAlternative().getEvaluationAt(criteria.get(1)),
						specimen.getAlternative().getEvaluationAt(criteria.get(2)));
			}
		}
